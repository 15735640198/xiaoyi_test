...
- **导航路径**: 设置 > 桌面、外屏和个性化
- **入口文本**: `桌面、外屏和个性化`（部分设备为`桌面和个性化`，需用子串`桌面`+`个性化`匹配）
- **滑动需求**: 2 屏
- **子项** (12 项):
...
| 壁纸 | nav_item (网格) | — |
| 锁屏 | nav_item (网格) | — |
| 熄屏显示 | nav_item (网格) | 进入子页面: Toggle(开关) + 显示方式(Radio: 智能显示/定时显示/全天显示) |
| 字体样式 | nav_item (网格) | — |
| 图标 | nav_item (网格) | — |
| 沉浸光感 | text_value | 右侧如"弱" |
| 桌面设置 | nav_item | 进入子页面（见下） |
| 锁屏设置 | nav_item | — |
| 熄屏显示设置 | nav_item | — |
| 智能显示 | nav_item | 进入熄屏显示子页面 |
| 更多 | nav_item | — |
| 设置锁屏签名 | nav_item | — |

**子页面: 桌面设置**

- **导航路径**: 设置 > 桌面、外屏和个性化 > 桌面设置
- **页面结构**: 多个 toggle_row 和 text_value 项

| 子项 | 形态 | 状态/操作说明 |
|------|------|------------|
| 桌面下滑 | toggle_row | checked → ON/OFF（子文本: 应用中心） |
| 桌面上滑 | toggle_row | checked → ON/OFF（子文本: 应用中心） |
| 桌面布局 | text_value | 右侧如"标准" |
| 自动对齐 | toggle_row | checked → ON/OFF（子文本: 删除应用后，自动补齐空位） |
| 负一屏 | text_value | 右侧"已开启"/"已关闭" |
| 锁定布局 | toggle_row | checked → ON/OFF（子文本: 开启后，桌面元素将无法移动或移除） |
| 图标调节 | nav_item | 进入子页面（调节图标尺寸和显示名称） |
